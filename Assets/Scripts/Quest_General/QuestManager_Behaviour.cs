...
public class QuestManager_Behaviour : MonoBehaviour {
    ...
    [SerializeField]
    Transform questDetailParent = null; //QuestのDetail(UI)をこのTransformの子として生成する QuestManagerの初期化用
    [SerializeField]
    DS.UI.Tab questTab = null;          //HeaderとDetailを動的生成した後にLinkTabHeaderを呼ぶため　QuestManagerの初期化用
    private void Start() {
        QuestManager.Init(questHeaderParent,questDetailParent,questTab);
    }
